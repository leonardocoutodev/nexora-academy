...
async function publicCoursePage(req,slug){
  ...
  const moduleCards=modules.map((m,index)=>`<article class="lc-public-module"><span>${String(index+1).padStart(2,"0")}</span><div><h3>${esc(m.title)}</h3>${m.description?`<p>${esc(m.description)}</p>`:""}<small>${Number(m.lesson_count||0)} aula(s)</small></div></article>`).join("");
  const canonical=`https://academy.learnandcreate.workers.dev/cursos/${encodeURIComponent(course.slug)}/`;
  const html=`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#07111F"><link rel="icon" href="/assets/brand/lc-mark.svg"><meta name="description" content="${esc(course.description||`Conheça o curso ${course.title} na LC.`)}"><link rel="canonical" href="${canonical}"><meta property="og:type" content="website"><meta property="og:site_name" content="LC — Learn & Create"><meta property="og:title" content="${esc(course.title)} — LC"><meta property="og:description" content="${esc(course.description||`Conheça o curso ${course.title} na LC.`)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="https://academy.learnandcreate.workers.dev/assets/brand/lc-mark.svg"><title>${esc(course.title)} — LC</title><link rel="stylesheet" href="/assets/css/lc.css"></head><body class="lc-public-page"><header class="public-header"><div class="container public-nav"><a class="brand" href="/"><img src="/assets/brand/lc-mark.svg" alt="LC"><span class="lc-brand-signature">Learn <span class="lc-brand-amp">&amp;</span> Create</span></a><nav class="public-links"><a href="/cursos/">Cursos</a><a href="/experimente/">Experimentar</a><a href="/programador-full-stack-pro/">LC Pro</a></nav><div class="public-actions"><a class="v3-btn secondary" href="/pages/login.html">Entrar</a><a class="v3-btn" href="/pages/cadastro.html?course=${encodeURIComponent(course.slug)}">Começar grátis</a></div></div></header><main><section class="container public-section"><div class="v3-hero"><div class="eyebrow">${esc(course.category_label||"FORMAÇÃO")} • ${esc(course.level_label||"LC FREE")}</div><h1>${esc(course.title)}</h1><p>${esc(course.description||"Formação prática disponível na LC.")}</p><div class="public-proof"><span><strong>${Number(course.module_count||0)}</strong><small>módulos</small></span><span><strong>${Number(course.lesson_count||0)}</strong><small>aulas</small></span><span><strong>${esc(course.minimum_score||70)}%</strong><small>nota mínima</small></span><span><strong>R$ 0</strong><small>acesso LC Free</small></span></div><div class="hero-actions"><a class="v3-btn" href="/pages/cadastro.html?course=${encodeURIComponent(course.slug)}">Começar gratuitamente →</a><a class="v3-btn secondary" href="/cursos/">Voltar ao catálogo</a></div></div></section><section class="container public-section"><div class="v3-section-head"><div><div class="eyebrow">CONTEÚDO</div><h2>O que você vai estudar</h2></div></div><div class="lc-public-modules">${moduleCards||'<div class="lc-admin-empty">Módulos em organização.</div>'}</div></section><section class="container public-section"><div class="formation"><div class="eyebrow">PRONTO PARA COMEÇAR?</div><h2>Crie sua conta gratuita e avance no seu ritmo.</h2><p class="muted">Aulas, atividades, avaliações e progresso ficam disponíveis dentro da plataforma.</p><a class="v3-btn" href="/pages/cadastro.html?course=${encodeURIComponent(course.slug)}">Criar conta gratuita →</a></div></section></main></body></html>`;
