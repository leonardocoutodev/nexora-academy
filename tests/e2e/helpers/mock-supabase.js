  ...
  await page.route('https://kvwsqfnyebyjncfgvqnd.supabase.co/**',async route=>{
    ...
    if(path==='/rest/v1/lessons'){
      if(url.searchParams.get('id')||url.search.includes('id=eq.')){
        ...
        if(url.search.includes(IDS.lessonDecision))return json(route,[{id:IDS.lessonDecision,module_id:IDS.module,title:'Tabela de decisão aplicada',objective:'Resolver uma regra de decisão em cenários normais, limítrofes e inválidos.',estimated_minutes:30,position:4,lab_type:'decision_table',lab_config:{decision_table:{title:'Decision Table Lab',scenario:'Classifique cada caso segundo a regra de aprovação.',outcomes:['Aprovar','Recusar','Revisar dados'],rows:[{case:'nota=8, frequência=90%',answer:0},{case:'nota=4, frequência=90%',answer:1},{case:'nota ausente, frequência=90%',answer:2}],feedback:'Tabela consistente.'}},xp_reward:100,difficulty:2,content:[{type:'concept',body:'Uma decisão robusta precisa tratar cada classe de entrada de forma explícita.'},{type:'recap',body:'Casos normais, limites e inválidos precisam de saída definida.'}]}]);
