...
for(const file of htmlFiles){
  ...
  const staticHtml=src.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi,'').replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi,'');
  if(!/<html[^>]+lang=["']pt-BR["']/i.test(staticHtml))failures.push(file+': lang pt-BR ausente');
  if(!/<meta[^>]+name=["']viewport["']/i.test(staticHtml))failures.push(file+': viewport ausente');
