...
for(const pattern of ["data-admin-panel=\"overview\"","data-admin-panel=\"students\"","data-admin-panel=\"boss\"","data-admin-panel=\"certificates\"","data-admin-panel=\"audit\"","studentDialog"]){if(!adminHtml.includes(pattern))failures.push(`admin/index.html: central operacional incompleta (${pattern})`)}
for(const pattern of ["admin_operational_summary","admin_student_roster","admin_student_detail","admin_update_profile","admin_update_enrollment","admin_create_enrollment","admin_boss_roster","admin_certificate_roster","admin_audit_feed","review_project_submission"]){if(!adminSource.includes(pattern))failures.push(`admin.js: operação administrativa ausente (${pattern})`)}
