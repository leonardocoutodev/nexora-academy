...
for(const pattern of ["lc.analytics.session","track_product_event","identify_analytics_session","lesson_engagement","lc:inline-check","lc:lab-complete","keepalive:true"]){if(!analyticsSource.includes(pattern))failures.push(`analytics.js: instrumentação ausente (${pattern})`)}
...
if(/LCAnalytics\.track\([^\n]{0,500}(?:email|password|full_name)/i.test([...files.filter(file=>file.endsWith(".html")||file.endsWith(".js")).map(file=>fs.readFileSync(file,"utf8"))].join("\n")))failures.push("analytics: possível PII enviada em propriedades de evento");
