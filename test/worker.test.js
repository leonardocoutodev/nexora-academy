...
test("health endpoint returns service status",async()=>{
  const response=await worker.fetch(new Request("https://nexora.test/api/health"),env,{});
...
test("protected endpoint rejects missing bearer token",async()=>{
  const response=await worker.fetch(new Request("https://nexora.test/api/nexora/me"),env,{});
  assert.equal(response.status,401);
});

test("static responses receive security and cache headers",async()=>{
  const response=await worker.fetch(new Request("https://nexora.test/pages/login.html"),env,{});
  assert.equal(response.status,200);
