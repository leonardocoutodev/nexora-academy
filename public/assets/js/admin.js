const AdminOps=(()=>{
  ...
  async function init(){
    ...
    try{
      ...
      const buildUtm=()=>{try{const url=new URL(qs('#growthUtmUrl').value.trim()||'https://academy.learnandcreate.workers.dev/');[['utm_source','growthUtmSource'],['utm_medium','growthUtmMedium'],['utm_campaign','growthUtmCampaign'],['utm_content','growthUtmContent']].forEach(([k,id])=>{const v=qs('#'+id).value.trim();if(v)url.searchParams.set(k,v);else url.searchParams.delete(k)});qs('#growthUtmResult').value=url.toString()}catch{qs('#growthUtmResult').value='URL base inválida'}};
      ['growthUtmUrl','growthUtmSource','growthUtmMedium','growthUtmCampaign','growthUtmContent'].forEach(id=>qs('#'+id)?.addEventListener('input',buildUtm));qs('#growthUtmCopy').onclick=async()=>{buildUtm();const v=qs('#growthUtmResult').value;if(!/^https?:\/\//.test(v))return toast('Corrija a URL base.','bad');try{await navigator.clipboard.writeText(v);toast('Link UTM copiado.')}catch{toast('Não foi possível copiar o link.','bad')}};buildUtm();
