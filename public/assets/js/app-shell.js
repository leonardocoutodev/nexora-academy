...
function applyLCNavigationIcons(root=document){
  const map={dashboard:'dashboard',courses:'courses',start:'start',projects:'projects',library:'library',certs:'certs',profile:'profile',affiliate:'affiliate'};
...
function enhanceMobileNavigation(active='dashboard'){
  ...
  const moreButton=qs('.nx-mobile-more-toggle',nav),moreActive=['library','certs','profile','affiliate'].includes(active);if(moreActive){moreButton.classList.add('active');moreButton.setAttribute('aria-current','page')}
