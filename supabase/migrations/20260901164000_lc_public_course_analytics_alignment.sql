-- Align analytics backend with the public course pages introduced in production.
-- Public course pages are intentionally accessible before authentication, so their
-- page-view event must be both allowlisted and accepted for anonymous sessions.
...
declare
  ...
  v_allowed constant text[]:=array[
    ...
    'course_opened','public_course_viewed',
...
begin
  ...
  if v_user is null and p_event_name not in (
    'signup_started','support_page_viewed','support_cta_clicked',
    'donation_started','donation_checkout_opened','donation_returned',
    'landing_page_viewed','landing_cta_clicked','public_course_viewed'
  ) then
